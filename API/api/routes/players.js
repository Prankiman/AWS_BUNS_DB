...
router.get("/:id", (req, res, next) => {
  ...
  if (req.params.id) {
    ...
    pool.query(
      `SELECT * FROM public."EdibleBuns" WHERE id = ${id}`,
      (err, response) => {
        if (err) {
          res.send(err);
          console.log(err);
...
router.get("/", (req, res) => {
  ...
  if (req.query.type) {
    let query = req.query.type.replace(/'/g, "");
    pool.query(
      `SELECT * FROM public."EdibleBuns" WHERE type = '${type}'`,
      (err, response) => {
        if (err) {
          res.send(err);
          console.log(err);
    ...
    pool.query('SELECT * FROM public."EdibleBuns"', (err, response) => {
      if (err) {
        res.send(err);
        console.log(err);
        return;
...
router.post("/", (req, res, next) => {
  ...
  pool.query(
    `INSERT INTO public."players" (name, birthdate, points, description, team_id) VALUES ('${req.body.name}'::character varying, '${req.body.birthdate}'::date, '${req.body.points}'::bigint, '${req.body.description}'::character varying, '${req.body.team_id}'::bigint) returning name;`,
    (err, response) => {
      if (err) {
        res.send(err);
        console.log(err);
...
router.patch("/:id", (req, res, next) => {
  ...
  pool.query(
    `UPDATE public."players" SET  (name, birthdate, points, description, team_id) = ('${req.body.name}'::character varying, '${req.body.birthdate}'::date, '${req.body.points}'::bigint, '${req.body.description}'::character varying, '${req.body.team_id}'::bigint)  WHERE id = ${req.params.id}`,
    (err, response) => {
      if (err) {
        res.send(err);
        console.log(err);
...
router.delete("/:id", (req, res, next) => {
  if (req.params.id) {
    let id = req.params.id;
    pool.query(
      `DELETE FROM public."players" WHERE id =  ${id}`,
      (err, response) => {
        if (err) {
          res.send(err);
          console.log(err);
